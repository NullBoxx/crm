...
<script type="text/javascript">
	...
	$(function(){
		...
		// 为搜索关联市场活动的模态窗口中的搜索框来绑定事件, 通过出发回车键, 来查询并展现所需市场活动列表
...
;
		...
		$("#bundBth").click(function () {
			...
			if ($xz.length == 0) {
				...
				// /workbench/clue/bund.do?clueId=xxx&activityId=xxx&clueId=xxx&activityId=xxx

				var param = "cid=${c.id}&";

				for (var i = 0; i < $xz.length; i ++) {

					param += "aid=" + $($xz[i]).val();  // $($xz[i]) 将dom对象包装成jq对象
					...
					success : function (data) {
						...
						* */
						if (data.success) {
							...
							// 清除搜索框中的信息 复选框中的勾干掉 清空activitySerachBody中的内容
		...
		<div class="modal-dialog" role="document" style="width: 80%;">
			<div class="modal-content">
				...
				<div class="modal-footer">
					...
					<button type="button" class="btn btn-primary" id="bundBth">关联</button>
				</div>
			</div>
		</div>
	</div>
